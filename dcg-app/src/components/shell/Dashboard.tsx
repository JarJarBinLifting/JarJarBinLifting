...
export function Dashboard({ onOpenUe, onNavigate, onQuickStart }: { onOpenUe: (ue: Ue) => void; onNavigate: (view: ShellView) => void; onQuickStart: (chapter: Chapter) => void }) {
  const { ues, chapters, qcmScores, timerSessions, dueChapters, dueFlashcards, dueQuiz, errorNotes, examDate, refreshAll } = useAppState();
  ...
  return (
    <div className="desktop-page">
      ...
      <section className="dashboard-metrics">
        ...
        <Metric value={dueErrors.length + dueToday.length + (dueFlashcards.total ? 1 : 0) + (dueQuiz.total ? 1 : 0)} label="actions utiles aujourd’hui" tone={dueErrors.length ? "red" : "blue"} />
      </section>
      ...
      <div className="dashboard-grid">
        <section className="surface work-queue">
          ...
          {dueQuiz.total > 0 && <button className="queue-row" onClick={() => setQuizOpen(true)}><span className="queue-dot" style={{ background: "var(--accent-cyan)" }} /><div><small>quiz éclair · QCM déjà ratés · sans IA</small><strong>{dueQuiz.total} question{dueQuiz.total > 1 ? "s" : ""} à reprendre</strong></div><span>Répondre →</span></button>}
          ...
          {!dueErrors.length && !dueToday.length && !dueFlashcards.total && !dueQuiz.total && <div className="queue-empty"><strong>Aucun retard à combler.</strong><p>Continue avec le prochain chapitre ou une session libre.</p><button className="soft-button" onClick={() => onNavigate("timer")}>Lancer une session</button></div>}
        </section>
